...
static void *extend_heap(size_t asize) 
{
    ...
    PUT(FTRP1(bp), PACK(size, 0));         /* Free block footer */   
...
static void alloc_block(void *bp,size_t size){
	...
	PUT(FTRP(bp),PACK((uint32_t)size,1));
}
/* 
...
static void print_heap(void *bp){
	...
	if(bp==heap_top)
return;
dbg_printf("heap_bottom:%p,bp:%p,heap_top:%p\n",heap_bottom,bp,heap_top);
	...
	for(;bp!=heap_top;bp=next_blkp(bp)){
		...
		printf("%p: header[%x:%x] footer[%x:%x]\n",
...
uint32_t pro_sizeh=GET_SIZE(HDRP(prologue));
	uint32_t pro_sizef=GET_SIZE(prologue);
	uint32_t epi_size=GET_SIZE(epilogue);
	uint32_t pro_alloch=GET_ALLOC(HDRP(prologue));
	uint32_t pro_allocf=GET_ALLOC(prologue);
	uint32_t epi_alloc=GET_ALLOC(epilogue);
dbg_printf("%p,%p",epilogue,heap_top);
	if (pro_sizeh!=8){
		dbg_printf("Error:bad prologue header size%u\n",pro_sizeh);
		return 1;
	}
	if (pro_sizef!=8){
	...
	if (pro_alloch!=1){
		dbg_printf("Error:bad prologue header alloc%u\n",pro_alloch);
		return 1;
	}
	if (pro_allocf!=1){
		dbg_printf("Error:bad prologue footer alloc%u\n",pro_allocf);
		return 1;
		}
	if (epi_alloc!=1){
		dbg_printf("Error:bad epi header%u\n",epi_alloc);
		return 1;
		}
	return 0;
	...
	if(verbose){
		...
		for(;get_succ_offset(bp)!=0;bp=succ_blkp(bp),count++){
			...
			if (bp!=pred_blkp(succ_blk)){
				dbg_printf("list not consis: block %d\n",count);
				return 1;
			}
		}
//		return 0;
...
static int check_list_block(int verbose){
	if (verbose){
		...
		for(;get_succ_offset(bp)!=0;bp=succ_blkp(bp),count++){
			alloc=GET_ALLOC(bp);
			if (alloc){
			dbg_printf("list has alloc block %d\n",count);
			return 1;
			}
		}
		//return 0;
