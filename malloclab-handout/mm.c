 ...
 * Based on (explicit list+ best fit);
 ...
 * The pred and succ part store the offset of the predecessor and
 * successor blocks in the free list.
 * Size classes: there are class_num size classes, the nth class stores 
...
static inline size_t get_palloc(void * bp){
return (GET(p)& 0x2);
}
...
static int check_my_heap(int verbose){
	if (verbose==0){
		...
		if ((size_t)heap_listp % 8){
			printf("prologue %p is not doubleword aligned\n", heap_listp);
			return 1;
		}
		if (GET(HDRP(heap_listp)) != GET(heap_listp)){
			printf("prologue: header does not match footer\n");
			return 1;
		}
		//check block
		free_block_num=0;
