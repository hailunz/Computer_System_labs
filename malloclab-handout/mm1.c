...
/* $begin mmmalloc */
	...
	newsize = 4*DSIZE;                                        //line:vm:mm:sizeadjust2
    else
	newsize = DSIZE * ((size + (DSIZE) + (DSIZE-1)) / DSIZE); //line:vm:mm:sizeadjust3
    

    /* Search the free list for a fit */
    if ((bp = find_fit(newsize)) != NULL) {  //line:vm:mm:findfitcall
	place(bp, newsize);                  //line:vm:mm:findfitplace
	return bp;
    }

    /* No fit found. Get more memory and place the block */
    extendsize = max(newsize,CHUNKSIZE);                 //line:vm:mm:growheap1
    if ((bp = extend_heap(extendsize/WSIZE)) == NULL)  
	return NULL;                                  //line:vm:mm:growheap2
    place(bp, newsize);                                 //line:vm:mm:growheap3
    in_heap(bp);
    return bp;
}
