...
/*
		...
		size_t next_alloc=GET_ALLOC((char *)ptr-oldsize+WSIZE);
		void * prev=PREV_BLKP(ptr);
		void *next=NEXT_BLKP(ptr);
		size_t prev_size=GET_SIZE((char *)prev-WSIZE);
		size_t next_size=GET_SIZE((char *)next-WSIZE);
		size_t coalsize=oldsize;
		...
		if (prev_alloc && next_alloc){
			...
			alloc_block(newptr,asize);
			set_free_block(NEXT_BLKP(newptr),(coalsize-asize),0);
			add_free_block(NEXT_BLKP(newptr));
			return newptr;
			}
		}*/
		newptr=malloc(size);
