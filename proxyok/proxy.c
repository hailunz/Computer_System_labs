...
//cache
cache *cache_head=NULL;//=(cache *)malloc(sizeof(cache));
...
void doit(int fd) 
{
	...
	cache_hit=search_cache_node(cache_head,uri);
	
if (cache_hit!=NULL){
		//cache hit
		cache_to_client(fd,cache_head,cache_hit);
		return;
	}
